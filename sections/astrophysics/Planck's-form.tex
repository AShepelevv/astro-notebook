...
\begin{center}
 \begin{tikzpicture}
  \begin{axis}[ymax=4.6e+14,xmax=1.8,axis x line=bottom,
axis y line=left, xlabel=Длина волны ({$\lambda,$} мкм), ylabel= Мощность излучения ({$B(\lambda),$} Вт/$\text{м}^2${$\cdot$}нм),width=7.5cm, height=7.5cm,legend pos = outer north east, no markers,minor x tick num = 1,minor y tick num = 1,grid = both,line width=1pt,cycle list = {
...
{smooth,brown,solid}
   ...
   \legend{5500K, 5000K, 4500K, 4000K, 3500K,}
  \end{axis}
 \end{tikzpicture}
\end{center}
